...
import {type Component, defineComponent, h, markRaw, nextTick, readonly, ref, shallowRef, watch,} from "vue";
...
const computeWrappedPageComponentName = (
    route: RouteLocationNormalizedLoadedGeneric | Page | string
): string => {
    if (typeof route === "string") {
        return "Page" + route.replace(/[^a-zA-Z0-9]/g, "-");
    }
...
const storeSetup = () => {
    ...
    watch(() => pages.value, () => {
        keepAliveInclude.value = pages.value.filter(needKeepAlive).map(computeWrappedPageComponentName);
    })
    ...
    const refreshPage = async () => {
        ...
        if (currentPage.meta?.['keepAlive']) {
            ...
            keepAliveInclude.value = keepAliveInclude.value
                .filter(name => name !== computeWrappedPageComponentName(route.fullPath));
